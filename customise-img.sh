...
cp -r conf firstrun.sh "${MOUNTPOINT}"
sed -i.orig '1s|$| systemd.run=/boot/firstrun.sh systemd.run_success_action=reboot systemd.unit=kernel-command-line.target|' "${MOUNTPOINT}/cmdline.txt"
